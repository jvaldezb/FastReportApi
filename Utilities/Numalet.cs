...
namespace Utilities
{
    public sealed class Numalet
    {
        ...
        {
            //String Numero2 = ReferenciaCultural.NumberFormat.NumberDecimalSeparator == "," ? 
            Double dNumero;
            //if (Double.TryParse(Numero, NumberStyles.Float, ReferenciaCultural, out dNumero))
            //if (!double.TryParse(Numero, System.Globalization.NumberStyles.Any, CultureInfo.CurrentCulture, out dNumero) &&
            //// Then try in US english
            //!double.TryParse(Numero, System.Globalization.NumberStyles.Any, CultureInfo.GetCultureInfo("en-US"), out dNumero) &&
            //// Then in neutral language
            //!double.TryParse(Numero, System.Globalization.NumberStyles.Any, CultureInfo.InvariantCulture, out dNumero))
            //if (Double.Parse(Numero.Replace(',', '.'), CultureInfo.InvariantCulture))


            //return ToCardinal(dNumero);
            dNumero = Double.Parse(Numero.Replace(',', '.'), CultureInfo.InvariantCulture);
            return ToCardinal(dNumero);
            //else throw new ArgumentException("'" + Numero + "' no es un número válido.");
        }
